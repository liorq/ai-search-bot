...
| קניבליזציה | `seo_core/sources/queries.py:find_cannibalisation()` | שני דפים על שאילתה; המנצח לפי קליקים; 70% השבה | `analyze_cannibalization` `נבדק בהרצה` — כל הדפים לשאילתה + ציון אנטרופיה; בלי מנצח, בלי אומדן, בלי פעולה | **לשמר ולתקן** — מוצע; המסנן שלנו יושב על הציר הלא נכון: 20 מ-22 ממצאים לא אמיתיים, ושניים מציעים 301 מדף הבית. *(נכתב כאן במקור "19" — מול הנתונים היומיים המספר הוא 20. ראה `docs/gsc-wizard-test/RESULTS.md` §2)* |
